...
/**
 * 智播专用 Camera2 深度定制推流与取景管线引擎
 * 核心特性：
 * 1. 强制将 Camera2 HAL 物理输出锁定为硬件原生 1920x1080 (16:9) 高清流，根除 HAL 降级为 640x480 / 1440x1440 的拉伸压扁缺陷
 * 2. SurfaceTexture Buffer 尺寸直接锁定为 1920x1080，彻底消灭未初始化 videoEncoder 尺寸的竞争陷阱
 * 3. 完美结合 PdkOpenGlView 渲染管线，实现 1:1 真实物理等比与沉浸式全屏
 */
@RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
public abstract class PdkCamera2Base {

    private static final String TAG = "PdkCamera2Base";
    ...
    public PdkCamera2Base(PdkOpenGlView openGlView) {
        context = openGlView.getContext();
        glInterface = openGlView;
        init(context);
    }

    public PdkCamera2Base(OpenGlView openGlView) {
        context = openGlView.getContext();
        glInterface = openGlView;
        init(context);
    }

    public PdkCamera2Base(Context context) {
    ...
    public void replaceView(PdkOpenGlView openGlView) {
        isBackground = false;
    ...
    private void startEncoders() {
        ...
        prepareGlView(cameraManager.getCurrentCameraId(), videoEncoder.getWidth(), videoEncoder.getHeight(), videoEncoder.getRotation());
    ...
    private Size getOptimalCameraSize(String cameraId, int targetW, int targetH) {
        int maxDim = Math.max(targetW, targetH);
        int minDim = Math.min(targetW, targetH);
        Size[] supported = (cameraId != null && !cameraId.isEmpty()) ? cameraManager.getCameraResolutions(cameraId) : null;
        if (supported != null && supported.length > 0) {
            for (Size s : supported) {
                if (s.getWidth() == maxDim && s.getHeight() == minDim) {
                    return s;
                }
            }
            float targetRatio = (float) maxDim / (float) minDim;
            Size best = null;
            float bestDiff = Float.MAX_VALUE;
            for (Size s : supported) {
                float r = (float) s.getWidth() / (float) s.getHeight();
                if (Math.abs(r - targetRatio) < 0.05f) {
                    float diff = Math.abs(s.getWidth() - maxDim) + Math.abs(s.getHeight() - minDim);
                    if (diff < bestDiff) {
                        bestDiff = diff;
                        best = s;
                    }
        ...
        return new Size(maxDim, minDim);
    }

    private void prepareGlView(int width, int height, int rotation) {
        prepareGlView(cameraManager.getCurrentCameraId(), width, height, rotation);
    }

    private void prepareGlView(String cameraId, int width, int height, int rotation) {
        int w = width;
        int h = height;
    ...
    public void setVideoBitrateOnFly(int bitrate) {
        Log.i(TAG, "setVideoBitrateOnFly: " + bitrate + " bps (" + (bitrate / 1000) + " kbps)");
